...
import { useGetFarmerOrdersQuery } from '../../slices/ordersApiSlice';
import Loader from '../../components/Loader';
import Message from '../../components/Message';
import { ClipboardList, CheckCircle, XCircle, ArrowRight } from 'lucide-react';

const FarmerOrderListPage = () => {
  const { userInfo } = useSelector((state) => state.auth);
  const { data: orders, isLoading, error } = useGetFarmerOrdersQuery();
  ...
  return (
    <div className='max-w-7xl mx-auto'>
      <div className='flex items-center gap-4 mb-10'>
        ...
        <div>
          <h1 className='text-4xl font-black text-gray-900'>{userInfo.isSupplier ? 'Supplier Orders' : 'Farmer Orders'}</h1>
          <p className='text-gray-500 font-medium'>View and manage orders for your products</p>
        </div>
      </div>
        ...
        <div className='bg-white rounded-[2.5rem] shadow-sm border border-gray-100 overflow-hidden'>
          ...
          {orders.length === 0 && (
            <div className='text-center py-20'>
              ...
              <p className='text-gray-500 font-medium'>When customers place orders for your products, they will appear here.</p>
            </div>
          )}
        </div>
      )}
